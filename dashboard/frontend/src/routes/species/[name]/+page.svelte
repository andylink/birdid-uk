...
<div class="h-[calc(100vh-3.25rem)] overflow-y-auto">
	<div class="max-w-5xl mx-auto px-6 py-5 space-y-6">
		...
		<a
			href={backHref}
			class="inline-flex items-center gap-1.5 text-xs text-slate-500 hover:text-slate-300
			       transition-colors"
		>
		...
		<div class="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden">
			...
			<div class="relative h-44 bg-slate-800 overflow-hidden">
				...
				{:else}
					...
					<div class="absolute inset-0 flex items-center justify-center">
						<svg viewBox="0 0 24 24" class="w-20 h-20 text-slate-700 fill-current" aria-hidden="true">
			...
			<div class="px-5 py-4">
				{#if statsLoading}
					<div class="h-7 bg-slate-800 rounded animate-pulse w-48 mb-2"></div>
					<div class="h-4 bg-slate-800 rounded animate-pulse w-32"></div>
				{:else if statsError}
					<p class="text-red-400 text-sm">{statsError}</p>
				{:else if stats}
					<h1 class="text-2xl font-bold text-slate-100 leading-tight">{stats.species}</h1>
				...
				<div class="bg-slate-900 border border-slate-800 rounded-lg divide-y divide-slate-800">
					{#each Array(8) as _}
						<div class="flex gap-3 px-4 py-3">
							<div class="w-1 h-12 bg-slate-800 rounded animate-pulse shrink-0"></div>
							<div class="flex-1 space-y-2">
								<div class="flex justify-between">
									<div class="h-4 bg-slate-800 rounded animate-pulse w-24"></div>
									<div class="h-4 bg-slate-800 rounded animate-pulse w-16"></div>
								</div>
								<div class="h-12 bg-slate-800 rounded animate-pulse"></div>
								<div class="h-8 bg-slate-800 rounded animate-pulse"></div>
							</div>
						</div>
					{/each}
				</div>
					...
					{#each detections as det (det.id)}
						...
						<div
							class="flex gap-3 px-4 py-3 transition-colors
							       {isNotable ? 'border-l-2 border-l-red-500 bg-red-950/10' : 'hover:bg-slate-800/40'}"
						>
							<!-- Confidence bar -->
							...
							<div class="flex-1 min-w-0">
								<div class="flex items-center justify-between gap-2 mb-1.5">
									<div class="flex items-center gap-2 flex-wrap">
										<time
											class="text-sm font-semibold text-slate-200 tabular-nums"
