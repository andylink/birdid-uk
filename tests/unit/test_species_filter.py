"""
Unit tests for species_filter.py.

The three-stage BOU matching logic is tested using a small synthetic BOU JSON
...
# Label map used in tests — keys are BirdNET common names, values are "sci_Common" strings.
_TEST_LABEL_MAP: dict[str, str] = {
    "Eurasian Wigeon":   "Mareca penelope_Eurasian Wigeon",    # stage-0 match
    "European Robin":    "Erithacus rubecula_European Robin",  # stage-1 match (scientific name)
    "Skylark":           "Alauda arvensis_Skylark",            # stage-2 match (common name fallback)
    "Common Buzzard":    "Buteo buteo_Common Buzzard",         # in label map but not in BOU JSON
    "Rüppell's Vulture": "Gyps rueppelli_Rüppell's Vulture",  # force-include test
}

# Synthetic BOU JSON covering all matching stages plus edge cases.
_TEST_BOU_JSON: list[dict] = [
    # Stage 0: has international_english_name that matches a label
    {
        "name": "Wigeon",
    ...
    # Stage 2: scientific name doesn't match any label, but BTO name "Skylark" does
    {
        "name": "Skylark",
        "scientific_name": "Alauda arvensis.FAKE",
        "british_list_status": "Resident Breeder",
    },
    # Excluded by status (no matching label anyway)
    {
        "name": "Cattle Egret",
        "scientific_name": "Bubulcus ibis",
        "british_list_status": "Accidental",
    },
    # Would be excluded by status, but force_include can override it
    {
    ...
    # Unmatched: scientific name and common name both absent from label_map
    {
        "name": "Imaginary Bird",
...
def bou_json_path(tmp_path: Path) -> Path:
    """Write the test BOU JSON to a temp file and return its path."""
...
def patch_bou_json(bou_json_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Replace _BOU_JSON with our test file for every test in this module."""
...
class TestStatusExcluded:
    ...
    def test_excluded_multi_token(self):
        # "Accidental" is one of two comma-separated tokens
    ...
    def test_slash_not_treated_as_separator(self):
        # "Passage/Winter Visitor" is one concept — the slash is not a delimiter
...
class TestBuildBouAllowedSet:
    def test_stage0_explicit_international_name(self):
        """international_english_name matching a label → stage-0 hit."""
    ...
    def test_stage2_common_name_fallback(self):
        """Species with a fake scientific name but matching BTO common name → stage-2 hit."""
        allowed = build_bou_allowed_set(_TEST_LABEL_MAP)
        assert "Skylark" in allowed

    def test_excluded_by_status(self):
        """Species with an excluded status should not appear in the allowed set."""
    ...
    def test_force_include_overrides_exclusion(self):
        """force_include should admit a species even when its status is excluded."""
        allowed = build_bou_allowed_set(
            _TEST_LABEL_MAP,
    ...
    def test_unmatched_species_absent(self):
        """Species with no label-map match at any stage should not appear."""
        allowed = build_bou_allowed_set(_TEST_LABEL_MAP)
        assert "Common Buzzard" not in allowed

    def test_empty_label_map_returns_empty_set(self):
        """An empty label map means nothing can be matched → empty frozenset."""
        allowed = build_bou_allowed_set({})
    ...
    def test_no_exclusions_by_default(self):
        """Without exclude_status, matchable Accidental species are still included."""
        allowed = build_bou_allowed_set(_TEST_LABEL_MAP)
        assert "Rüppell's Vulture" in allowed
